...
It was designed as a **production-style secure AI service**, not just a notebook project. The system includes:
...
How It Works
A user sends suspicious text to the /predict endpoint.
The API validates and cleans the input.
The trained model predicts whether the text is phishing or legitimate.
The API returns:
input text
prediction
confidence score
The endpoint is protected with API key authentication and rate limiting.
...
git commit -m "Add project README with screenshots"
git push
